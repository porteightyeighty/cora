...
/**
 * An ExtendableSubstitution is a substitution that can have mappings added over time, though
 ...
 */
public class ExtendableSubstitution implements Substitution {
  ...
  /**
   * Creates an empty extendable substitution, with neither variables nor type variables in its
   * domain.
   */
  public ExtendableSubstitution() {
  ...
  /**
   * Creates an extendable substitution [x:=s].
   *
   ...
   */
  public ExtendableSubstitution(Replaceable x, Term s) {
    _mapping = new HashMap<Replaceable,Term>();
    _typeMapping = new HashMap<TVar,Type>();
    _unmodifiableTypeMapping = Collections.unmodifiableMap(_typeMapping);
    extend(x, s);
  ...
  /** Creates an extendable substitution that copies the given substitution. */
  public ExtendableSubstitution(Substitution copyme) {
    if (copyme instanceof ExtendableSubstitution gamma) {
      _mapping = new HashMap<Replaceable,Term>(gamma._mapping);
      _typeMapping = new HashMap<TVar,Type>(gamma._typeMapping);
    }
    else {
      _mapping = new HashMap<Replaceable,Term>();
      _typeMapping = new HashMap<TVar,Type>();
      for (TVar x : copyme.typeDomain()) _typeMapping.put(x, copyme.get(x));
      for (Replaceable z : copyme.domain()) _mapping.put(z, copyme.get(z));
    }
    _unmodifiableTypeMapping = Collections.unmodifiableMap(_typeMapping);
  }
  ...
  public boolean extend(TVar key, Type value) {
    if (key == null) throw new NullStorageException("ExtendableSubstitution", "type key");
    if (value == null) throw new NullStorageException("ExtendableSubstitution", "type value");
  ...
  public boolean extend(Replaceable key, Term value) {
    if (key == null) throw new NullStorageException("ExtendableSubstitution", "key");
    if (value == null) throw new NullStorageException("ExtendableSubstitution", "value");
  ...
  public Term applySubstitution(Term term) {
    ...
    else throw new IllegalArgumentException("ExtendableSubstitution::applySubstitution called " +
  ...
  private Term substituteMetaApplication(MetaVariable z, ArrayList<Term> args) {
    ...
    ExtendableSubstitution delta = new ExtendableSubstitution();
